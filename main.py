...
# фильтр таблицы
rslt_df = df[(df.отношение == 'положительное') & (df.пол == 'муж') & (df.возраст == 'до 35 лет')] 
rslt_df = df[(df.отношение == 'положительное') & (df.пол == 'муж') & (df.возраст == age)]
st.dataframe(rslt_df)
st.dataframe(rslt_df["Какие событийные мероприятия, по Вашему мнению, будут интересны жителям и гостям города?"])
